...
import {
  getRoles, getRolePermissions, updateRolePermissions,
...
function RoleCard({
  ...
  onRemovePerm, onSave, saving, saveError, saveSuccess,
  onRename, onDelete,
}: {
  role: RoleItem;
  permissions: string[];
  ...
  saveSuccess: boolean;
  onRename: (slug: string, newLabel: string, newSlug: string) => Promise<string | null>;
  ...
  async function submitRename() {
    ...
    const newSlug = slugValue.trim().toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "");
    ...
    >
        ...
        {editing ? (
            ...
            <div className="flex items-center gap-2">
              <div className="flex flex-1 flex-col gap-1.5">
                <div className="flex items-center gap-1.5">
                  <span className="w-12 shrink-0 text-right text-xs text-neutral-400">Label</span>
                  <input
                    ref={labelRef}
                    value={labelValue}
                ...
                <div className="flex items-center gap-1.5">
                  <span className="w-12 shrink-0 text-right text-xs text-neutral-400">Slug</span>
                  <input
                    value={slugValue}
                    onChange={(e) => setSlugValue(e.target.value.toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, ""))}
                    onKeyDown={(e) => { if (e.key === "Enter") submitRename(); if (e.key === "Escape") setEditing(false); }}
                ...
                {editError && <p className="text-xs text-red-600 pl-14">{editError}</p>}
              </div>
              <div className="flex flex-col gap-1">
                <button onClick={submitRename} disabled={editSaving} className="rounded p-1 text-brand-orange hover:bg-orange-50 disabled:opacity-60">
                  <Check className="h-4 w-4" />
                </button>
                <button onClick={() => { setEditing(false); setLabelValue(role.label); setSlugValue(role.slug); setEditError(null); }} className="rounded p-1 text-neutral-400 hover:bg-neutral-100">
                  <X className="h-4 w-4" />
                </button>
              </div>
            </div>
          ...
          <div className="flex flex-1 items-center gap-1.5">
            <h3 className="text-sm font-semibold text-brand-dark">
              ...
              {role.isSystem && (
                <span className="ml-1.5 text-xs font-normal text-neutral-300">system</span>
              )}
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          {deleteConfirm && (
            <div className="flex items-center gap-1.5 text-xs">
              ...
              <button onClick={confirmDelete} disabled={deleting} className="font-medium text-red-600 hover:underline disabled:opacity-60">
                {deleting ? "…" : "Yes"}
              </button>
              <button onClick={() => setDeleteConfirm(false)} className="text-neutral-400 hover:underline">No</button>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="min-h-[2.5rem]">
        {permissions.length === 0 ? (
          <p className="text-xs text-neutral-400 italic">Drop permissions here</p>
        ) : (
...
export default function RolesTab() {
  ...
  function handleDragEnd(e: DragEndEvent) {
    ...
    if (activeId.startsWith("available:")) {
      ...
      setPermissions((prev) => ({ ...prev, [sourceSlug]: prev[sourceSlug].filter((p) => p !== perm) }));
    } else return;
  ...
  async function handleSave(slug: string) {
    ...
    try {
      const { permissions: saved } = await updateRolePermissions(token!, slug, permissions[slug] ?? []);
      setSavedPermissions((p) => ({ ...p, [slug]: saved }));
      setSaveSuccess((p) => ({ ...p, [slug]: true }));
      if (successTimers.current[slug]) clearTimeout(successTimers.current[slug]);
      successTimers.current[slug] = setTimeout(() => setSaveSuccess((p) => ({ ...p, [slug]: false })), 3000);
    } catch (err) {
      setSaveError((p) => ({ ...p, [slug]: err instanceof ApiError ? err.message : "Save failed." }));
    } finally {
  ...
  async function handleAddRole() {
    ...
    try {
      ...
      setAddError(err instanceof ApiError ? err.message : "Failed to create role.");
    } finally {
      setAddingRole(false);
    }
  }

  async function handleRename(oldSlug: string, label: string, newSlug: string): Promise<string | null> {
    try {
      const { role: updated } = await renameRole(token!, oldSlug, { slug: newSlug, label });
      setRoleList((prev) => prev.map((r) => (r.slug === oldSlug ? updated : r)));
  ...
  if (loading) return <div className="text-sm text-neutral-500 py-6">Loading roles…</div>;
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start">
        ...
        <div className="flex-1 min-w-0 space-y-4">
          ...
          <div className="rounded-xl border border-neutral-200 bg-white p-5 shadow-sm">
            <h2 className="text-base font-semibold text-brand-dark">Roles & Permissions</h2>
            <p className="text-sm text-neutral-500 mt-0.5">
              Drag permissions onto a role, click × to remove, then Save. Rename or delete custom roles with the inline controls.
            </p>
            ...
            <div className="mt-4 flex items-center gap-2">
              <input
                value={newRoleLabel}
                onChange={(e) => { setNewRoleLabel(e.target.value); setAddError(null); }}
        ...
        <div className="w-full lg:w-60 shrink-0">
          <div className="sticky top-24 rounded-xl border border-neutral-200 bg-white p-5 shadow-sm">
            <h3 className="text-sm font-semibold text-brand-dark mb-1">All Permissions</h3>
            <p className="text-xs text-neutral-400 mb-4">Drag any permission onto a role card.</p>
            <div className="flex flex-col gap-1.5">
              {ALL_PERMISSIONS.map((perm) => (
                <DraggableChip key={perm} id={`available:${perm}`} label={perm} />
              ))}
            </div>
          </div>
        </div>
